...
from ..utils.text import (
    chat_context_prefix, current_time_text, strip_think_tags, 
    merge_reply_segments, preview_text, format_cn_date, format_local_datetime
)
from ..utils.shell import parse_json_output
from ..utils.env import (
    load_json_file, expand_env_placeholders, load_workflow_defaults,
    WORKFLOW_CONFIG_PATH, DEFAULT_WORKFLOW_PATH, INTENT_EXTRACTOR_CONFIG_PATH
)
from ..models.config import Config
from ..models.history import build_storage_config
from .intent import (
    wants_explicit_search, coerce_to_local_find_payload,
...
class OpenAICompatibleAgent:
    ...
    def _summarize_remember_output(
        ...
        if not extracted_memories:
            ...
            if memory_value:
                formatted_time = format_local_datetime(created_at) if created_at else "刚刚"
                extracted_memories.append(f"- {memory_value} (记录时间: {formatted_time})")
