...
if [ -f "$PB_BIN" ] && ! "$PB_BIN" --version >/dev/null 2>&1; then
    echo "Existing PocketBase binary is not executable on this platform; downloading a compatible copy..." >&2
    rm -f "$PB_BIN"
fi

if [ ! -f "$PB_BIN" ]; then
    echo "Downloading PocketBase v$PB_VERSION for $OS $ARCH..." >&2
...
echo "Starting PocketBase on 0.0.0.0:18090 using data dir $PB_DATA_DIR..." >&2
