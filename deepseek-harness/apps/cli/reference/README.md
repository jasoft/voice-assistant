...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default. Pass `--host 0.0.0.0` for an explicit LAN deployment; the web runtime derives the machine's LAN IP literals for the `/api` browser-trust fence. `--trusted-host` adds named authorities accepted by that fence.
