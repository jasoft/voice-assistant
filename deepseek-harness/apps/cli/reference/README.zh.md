...
生产 Web 运行器需要已构建的包和前端产物（`pnpm run build`）。默认服务地址是 `http://127.0.0.1:3080`。显式传入 `--host 0.0.0.0` 即可启用局域网部署；Web 运行时会将本机 LAN IP 字面量推导到 `/api` 浏览器信任围栏中。`--trusted-host` 可添加该围栏接受的具名 authority。
