...
2. 尽量复用现有接口。继续保留 `remember_add` / `remember_find` / `remember_list` 三个工具名，避免动意图层和工作流配置。
...
```python
client.get_all(filters={"user_id": "soj"}, limit=limit)
```

再把结果按统一解析逻辑转成可供总结的文本。这里不引入本地缓存，也不做二次持久化。
