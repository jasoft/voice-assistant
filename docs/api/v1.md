...
**Logic Note (Harness mode):** A photo is passed to the `memo-minimal` Agent with the query. The Agent saves it as a memory only when the request explicitly asks to remember/save/record it.
...
#### Routing
- Explicit “记住 / 记录 / 保存” requests are written to Memos without rewriting the original text.
- Questions about personal memories are answered from one Memos recall operation.
- General questions are answered directly; current events may trigger at most one Brave `web_search`.

#### Guarantees
Each request creates a fresh session, returns one final reply, does not ask follow-up questions, and carries no prior conversation context. The synchronous deadline defaults to 30 seconds (`PTT_CHAT_TIMEOUT_SECONDS`). Successful replies are persisted to session history with `mode: chat`.
...
- **Harness-mode Response:** Array of all current-user Mem0 records（遗留 Mem0 后端，记忆主链路已切换为 Memos），descending by creation time.
