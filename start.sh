...
# DeepSeek Harness owns query orchestration and Mem0 access; PocketBase remains
# the durable history archive required by /v1/history.
...
if [[ "$QUERY_BACKEND" == "harness" || "$QUERY_BACKEND" == "deepseek-harness" ]]; then
    echo "Starting ptt-api with DeepSeek Harness queries, Mem0 memories, and PocketBase history" >&2
else
    echo "Starting ptt-api with legacy PocketBase storage" >&2
fi
